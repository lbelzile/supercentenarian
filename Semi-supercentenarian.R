...
# and for two-sided tests with Ha gamma=0 versus H0: gamma != 0
...
slow <- as.numeric(pmax(0, c1 - xcal))
# Exceedances
datu <- italcent$numdays - u
rightcens <- italcent$rightcens
# Fit a generalized Pareto model
ind <- which(datu > 0)
uplim <- as.numeric(c2 - italcent$birth)
# Keep only exceedances above 108
uplim <- (uplim[ind] - u)/365.25
rcens <- rightcens <- rightcens[ind]
datu <- datu[ind]/365.25
slow <- slow[ind]/365.25



power <- array(0, dim = c(B, nxis, 2))
bootsamp <- matrix(0, ncol = length(datu), nrow = B)
rightcensb <- matrix(FALSE, ncol = length(datu), nrow = B)
profile_italcent <- t(sapply(1:nxis, function(i){prof_gpd_cens_xi(xi = xis[i], dat = datu, rightcens, slow)}))[,3:2]
for(i in 1:nxis){
  #Generate new data
  prof_xi_i <- prof_gpd_cens_xi(xi = xis[i], dat = datu, rightcens, slow)
  for(j in 1:ncol(bootsamp)){
    bootsamp[,j] <- evd::qgpd(evd::pgpd(slow[j], loc = 0, scale = prof_xi_i[3], shape = prof_xi_i[2]) + runif(B)*
                                (1-evd::pgpd(slow[j], loc = 0, scale = prof_xi_i[3], shape = prof_xi_i[2])), loc = 0, scale = prof_xi_i[3], shape = prof_xi_i[2])
    rightcensb[,j] <- bootsamp[,j] > uplim[j]
    bootsamp[rightcensb[,j],j] <- uplim[j]
  }
  for(b in 1:B){
    #Find maximum likelihood estimates
    mleboot <- try(optim(par =  prof_xi_i[3:2],
                         fn = gpd_cens,
                         method = "N",
                         dat = bootsamp[b,],
                         ...
                         hessian = TRUE))
    if(is.character(mleboot)){
      power[b,i,] <- rep(NA, 2)
    } else{
      #Compute directed  profile likelihood ratio root test statistic
      power[b, i, 1] <- sign(mleboot$par[2])*
        sqrt(rfun_xi_gpd_cens(psi = 0, 
                              thetahat = mleboot$par, 
                              dat = bootsamp[b,],
                              rightcens = rightcensb[b,],
                              slow = slow))
      #Compute Wald-test
      if(mleboot$par[2] > -0.5){
        power[b,i,2] <- mleboot$par[2]/sqrt(diag(solve(mleboot$hessian))[2])
      } else{ 
        power[b,i,2] <- NA
      }
    }
...
lrt_twoside <- abs(power[,,2]) > qnorm(0.975)
wald_twoside <- abs(power[,,1]) > qnorm(0.975)
plot(xis, colMeans(lrt_twoside, na.rm = TRUE), type = "l",
     yaxs = "i", ylim = c(0,1), bty = "l", 
     ylab = "power")
lines(xis, colMeans(wald_twoside, na.rm = TRUE), col = 2)

lrt_oneside <- power[,,2] > qnorm(0.95)
wald_oneside <- power[,,1] > qnorm(0.95)
plot(xis, colMeans(lrt_oneside, na.rm = TRUE), type = "l", col = 4, 
     yaxs = "i", ylim = c(0,1), xlim = c(0, max(xis)), bty = "l", 
     ylab = "power")
lines(xis, colMeans(wald_oneside, na.rm = TRUE), col = 2)

if(save){
  save(xis, power, file = "power_italcent_108.RData")
}
## Same for France at 108
...
francent$slow <- ifelse(francent$numdays > 110*365.24, 
                        ...
                        as.numeric(pmax(0, c1a - xcal)))
francent$supp <- as.numeric(pmax(0, c2 - xcal))    
datu <- francent$numdays - u
ind <- which(datu > 0)
slow <- francent$slow[ind]/365.25
supp <- francent$supp[ind]/365.25
datu <- (francent$numdays - u)[ind]/365.25

set.seed(20200824)
power <- array(0, dim = c(B, nxis, 2))
bootsamp <- matrix(0, ncol = length(datu), nrow = B)
rightcensb <- matrix(FALSE, ncol = length(datu), nrow = B)
prof_xi <- prof_gpd_dtrunc_xi(xi = xis,
                              dat = datu, 
                              supp = supp, 
                              slow = slow)
profile_francent <- prof_xi$param[,1:2]
for(i in 1:nxis){
  #Generate new data (doubly censored)
  for(j in 1:ncol(bootsamp)){
    bootsamp[,j] <- revddtrunc(n = B, param = prof_xi$param[i,], 
                               lower = slow[j], 
                               upper = supp[j])
  }
  for(b in 1:B){
    #Find maximum likelihood estimates
    mleboot <- try(optim(par =  prof_xi$param[i,],
                         fn = gpd_dtrunc,
                         method = "N",
                         dat = bootsamp[b,],
                         supp = supp,
                         ...
                         control = list(fnscale = c(1.5,0.01), reltol = 1e-12, maxit= 1e5),
                         hessian = TRUE))
    mboot <- mean(bootsamp[b,])
    pexpboot <- try(optim(par = mboot, 
                          fn = gpd_dtrunc, 
                          method = "Brent", 
                          lower = 0.01*mboot, 
                          upper = 3*mboot, 
                          ...
                          expo = TRUE, 
                          hessian = TRUE))
    if(is.character(mleboot) || is.character(pexpboot)){
      power[b,i,] <- rep(NA, 2)
    } else{
      #Compute directed profile likelihood ratio root test statistic
      power[b, i, 1] <- sign(mleboot$par[2])*
        sqrt(2*(pexpboot$value - mleboot$value))
      #Compute Wald-test
      if(mleboot$par[2] > -0.5){
        power[b,i,2] <- mleboot$par[2]/sqrt(diag(solve(mleboot$hessian))[2])
      } else{ 
...
if(save){
  save(xis, power, file = "power_francent_108.RData")
}
...
if(save){
  save(xis, power, file = "power_IDL_110.RData")
}

#TODO compute power at 107 and 108.


load("power_IDL_110.RData")
powerIDL <- colMeans(cbind(abs(power[,,2]) > qnorm(0.975),
                           abs(power[,,1]) > qnorm(0.975),
                           power[,,2] < qnorm(0.05),
                           power[,,1] < qnorm(0.05)), na.rm = TRUE)


load("power_italcent_108.RData")
powerIstat <- colMeans(cbind(abs(power[,,2]) > qnorm(0.975),
                             abs(power[,,1]) > qnorm(0.975),
                             power[,,2] < qnorm(0.05),
                             power[,,1] < qnorm(0.05)), na.rm = TRUE)

load("power_francent_108.RData")
powerfrancent <- colMeans(
  cbind(abs(power[,,2]) > qnorm(0.975),
        abs(power[,,1]) > qnorm(0.975),
        power[,,2] < qnorm(0.05),
        power[,,1] < qnorm(0.05)), na.rm = TRUE)
...
power_df <- data.frame(
  shape = rep(xis, length.out = 12*nxis),
  test = factor(rep(rep(c("Wald","directed likelihood root"), each = nxis), length.out = 12*nxis)),
  data = factor(rep(c("Istat","France","IDL2016"), each = 4*nxis)),
  hypothesis = factor(rep(rep(c("two-sided","one-sided"), each = 2*nxis), length.out = 12*nxis)),
  power = c(powerIstat, powerfrancent, powerIDL))

if(save){
  save(power_df, file = "power_combined.RData")
}
library(ggplot2)
library(poorman)
library(viridis)
...
g1 <- power_df %>% filter((hypothesis == "two-sided")&(test != "Wald")) %>%
  ggplot(aes(x = shape, y = power, col = data)) + 
  geom_hline(yintercept = 0.05, alpha = 0.5) + 
  geom_smooth(method = "gam", se = FALSE,
              formula = y ~ s(x, bs = "tp", fx = TRUE, k=25)) +
  # geom_line() + 
  scale_color_viridis_d() + 
  theme_classic() + 
  theme(legend.position = "bottom", panel.grid.major = element_line())  + #seq(0,1, by = 0.1)) +
  ...
  scale_x_continuous(breaks = seq(-0.25,0.25, by = 0.05),
                     limits = c(-0.25,0.25),
                     labels = paste0("$",seq(-0.25,0.25, by = 0.05),"$")) + 
  xlab("$\\gamma$")
if(figures){
  fig <- "Fig9a.tex"
  setwd(fig_dir)
...
power_df[sapply(power_df$shape, function(x){isTRUE(all.equal(x,-0.1))}) & power_df$test == "Wald",][c(2,4,6),5]
# Combined power
1-prod(1-power_df[sapply(power_df$shape, function(x){isTRUE(all.equal(x,-0.1))}) & power_df$test == "Wald",][c(2,4,6),5])
1-prod(1-power_df[sapply(power_df$shape, function(x){isTRUE(all.equal(x,-0.05))}) & power_df$test == "Wald",][c(2,4,6),5])

#########################################################################################
#########################################################################################
# Power plots (Figure 3)
# Repeat the calculations of the previous section with the likelihood ratio test
# but this time using an equispaced grid of endpoint values
# Merge the result with the Wald test for gamma < 0 from the previous code chunk
set.seed(20200618)
B <- 1e4


u <- 39447L #108 years
endpoints <- c(seq(115, 125, by = 0.2), seq(126, 180, by = 1))
...
profile_italcent <- t(sapply(1:nendpoints, function(i){
  prof_gpd_cens_endpoint(endpoint = (endpoints[i] - 108), dat = datu, rightcens, slow)
}))

if(save){
  power_italcent <- matrix(0, nrow = B, ncol = nendpoints)
  bootsamp <- matrix(0, ncol = length(datu), nrow = B)
  rightcensb <- matrix(FALSE, ncol = length(datu), nrow = B)
  
  # plot(108-profile_italcent[,2]/profile_italcent[,3], profile_italcent[,1])
  for(i in 1:nendpoints){
    #Generate new data
    if(endpoints[i] < (max(datu) + 108)){
      power_italcent[,i] <- -Inf
    } else{
      prof_xi_i <- profile_italcent[i,2:3]
      for(j in 1:ncol(bootsamp)){
        bootsamp[,j] <- evd::qgpd(evd::pgpd(slow[j], loc = 0, scale = prof_xi_i[1], shape = prof_xi_i[2]) + runif(B)*
                                    (1-evd::pgpd(slow[j], loc = 0, scale = prof_xi_i[1], shape = prof_xi_i[2])), loc = 0, scale = prof_xi_i[1], shape = prof_xi_i[2])
        rightcensb[,j] <- bootsamp[,j] > uplim[j]
        bootsamp[rightcensb[,j],j] <- uplim[j]
      }
      for(b in 1:B){
        #Find maximum likelihood estimates
        mleboot <- try(optim(par =  prof_xi_i,
                             fn = gpd_cens,
                             method = "N",
                             dat = bootsamp[b,],
                             rightcens = rightcensb[b,],
                             ...
                             hessian = TRUE))
        if(is.character(mleboot)){
          power_italcent[b, i] <- NA
        } else{
          #Compute directed  profile likelihood ratio root test statistic
          power_italcent[b, i] <- sign(mleboot$par[2])*
            sqrt(rfun_xi_gpd_cens(psi = 0, 
                                  thetahat = mleboot$par, 
                                  dat = bootsamp[b,],
                                  rightcens = rightcensb[b,],
  ...
  # lrt_oneside <- power_italcent > qnorm(0.95)
  # plot(xis, colMeans(lrt_oneside, na.rm = TRUE), type = "l", col = 4, 
  #      yaxs = "i", ylim = c(0,1), xlim = c(0, max(xis)), bty = "l", 
  #      ylab = "power")
  
  save(endpoints, power_italcent, file = "power_endpoint.RData")
}
## Same for France at 108
...
profile_francent <- t(sapply(1:nendpoints, function(i){
  prof_gpd_dtrunc_endpoint(endpoint = (endpoints[i] - 108), dat = datu, slow = slow, supp = supp)
}))
if(save){
  set.seed(20200824)
  power_francent <- array(0, dim = c(B, nendpoints))
  bootsamp <- matrix(0, ncol = length(datu), nrow = B)
  rightcensb <- matrix(FALSE, ncol = length(datu), nrow = B)
  
  for(i in 1:nendpoints){
    if(endpoints[i] < (max(datu) + 108)){
      power_francent[,i] <- -Inf
    } else{
      #Generate new data (doubly censored)
      for(j in 1:ncol(bootsamp)){
        bootsamp[,j] <- revddtrunc(n = B, param = profile_francent[i,2:3], 
                                   lower = slow[j], 
                                   upper = supp[j])
      }
      for(b in 1:B){
        #Find maximum likelihood estimates
        mleboot <- try(optim(par =  profile_francent[i,2:3],
                             fn = gpd_dtrunc,
                             method = "N",
                             dat = bootsamp[b,],
                             supp = supp,
        ...
        pexpboot <- try(optim(par = mboot, 
                              ...
                              hessian = TRUE))
        if(is.character(mleboot) || is.character(pexpboot)){
          power_francent[b,i] <- NA
        } else {
          #Compute directed profile likelihood ratio root test statistic
          power_francent[b, i] <- sign(mleboot$par[2])*sqrt(2*(pexpboot$value - mleboot$value))
        }
      }
    }
  }
  save(endpoints, power_italcent, power_francent, file = "power_endpoint.RData")
}


load("IDL2016.rda")
#Remove French to avoid overlap with France 2019
idl2016 <- idl2016[!idl2016$countrydeath == "FRA",]
u110 <- min(idl2016$numdays)-1 #some people are 2-3 days from 110...
# Calendar date at which individual reaches 105 years
datu <- idl2016$numdays - u110
...
profile_idl <- t(sapply(1:nendpoints, function(i){
  prof_gpd_dtrunc_endpoint(endpoint = (endpoints[i] - 110), dat = datu, slow = slow, supp = supp)
}))
if(save){
  set.seed(20200824)
  power_idl <- array(0, dim = c(B, nendpoints))
  bootsamp <- matrix(0, ncol = length(datu), nrow = B)
  rightcensb <- matrix(FALSE, ncol = length(datu), nrow = B)
  
  for(i in 1:nendpoints){
    if((endpoints[i] - 110) <= max(datu) ){
      power_idl[,i] <- -Inf
    } else{
      #Generate new data (doubly censored)
      for(j in 1:ncol(bootsamp)){
        bootsamp[,j] <- revddtrunc(n = B, param = profile_idl[i,2:3], 
                                   lower = slow[j], 
                                   upper = supp[j])
      }
      for(b in 1:B){
        #Find maximum likelihood estimates
        mleboot <- try(optim(par =  profile_idl[i,2:3],
                             fn = gpd_dtrunc,
                             method = "N",
                             dat = bootsamp[b,],
                             supp = supp,
        ...
        if(is.character(mleboot) || is.character(pexpboot)){
          ...
          #Compute directed profile likelihood ratio root test statistic
          power_idl[b, i] <- sign(mleboot$par[2])*sqrt(2*(pexpboot$value - mleboot$value))
        }
      }
    }
  }
  save(endpoints, power_italcent, power_francent, power_idl, file = "power_endpoint.RData")
}
...
powerIDL <- colMeans(power_idl < qnorm(0.05), na.rm = TRUE)
powerIstat <- colMeans(power_italcent < qnorm(0.05), na.rm = TRUE)
powerFrance <- colMeans(power_francent < qnorm(0.05), na.rm = TRUE)
powerCombo <- 1-(1-powerIstat)*(1-powerFrance)*(1-powerIDL)

power_df <- data.frame(
  endpoint = rep(endpoints, length.out = 3*length(endpoints)),
  data = factor(rep(c("Istat","France","IDL2016"), each = length(endpoints))),
  power = c(powerIstat, powerFrance, powerIDL))

# Smoothing of the power curve to get rid of the artifacts of Monte-Carlo variability
library(cobs)
nknots <- 40L
endpts <- seq(115, 180, by = 0.1)
p1IT <- (1:(max(which(powerIstat == 1)-1L)))
powersmoothIstat <- predict(cobs::cobs(
  x = endpoints[-p1IT], 
  y = powerIstat[-p1IT], 
  constraint = "decrease", 
  nknots = nknots),z = endpts)[,"fit"]
powersmoothIstat[endpts < endpoints[max(which(powerIstat == 1))]] <- 1

p1FR <- (1:(max(which(powerFrance == 1))-1L))
powersmoothFrance <- predict(
  cobs::cobs(x = endpoints[-p1FR], 
             ...
             nknots = nknots),
  z = endpts)[,"fit"]
powersmoothFrance[endpts < endpoints[max(which(powerFrance == 1))]] <- 1


p1IDL <- (1:(max(which(powerIDL == 1)-1L)))
powersmoothIDL <- predict(cobs::cobs(
  x = endpoints[-p1IDL], 
  y = powerIDL[-p1IDL], 
  constraint = "decrease", nknots = nknots),
  z = endpts)[,"fit"]
powersmoothIDL[endpts < endpoints[max(which(powerIDL == 1))]] <- 1
powersmoothCombo <- 1-(1-powersmoothIstat)*(1-powersmoothFrance)*(1-powersmoothIDL)
power_endp <- data.frame(
  endpoint = rep(endpts, length.out = 3*length(endpts)),
  data = factor(rep(c("Istat","France","IDL2016"), each = length(endpts))),
  power = c(powersmoothIstat, powersmoothFrance, powersmoothIDL)
)
power_endp$power <- pmin(pmax(0, power_endp$power), 1)
power_endp$lower <-  pmax(0,power_endp$power - qnorm(0.975)*sqrt(power_endp$power*(1-power_endp$power)/B))
power_endp$upper <-  pmin(1,power_endp$power + qnorm(0.975)*sqrt(power_endp$power*(1-power_endp$power)/B))

power_endp_combo <- data.frame(endpoint = endpts,
                               power = powersmoothCombo)
...
load(file = "power_combined.RData")
power_gamma_wald <-  power_df %>% filter((hypothesis == "one-sided")&(test == "Wald"))
power_gamma_wald_combo <- data.frame(gamma = power_gamma_wald$shape[power_gamma_wald$data == "IDL2016"],
                                     power = 1- (1-power_gamma_wald$power[power_gamma_wald$data == "IDL2016"])*(1-power_gamma_wald$power[power_gamma_wald$data == "France"])*(1-power_gamma_wald$power[power_gamma_wald$data == "Istat"]))

if(figures){
  fig <- "Fig10.tex"
  setwd(fig_dir)
  tikz(fig, width = 8, height = 4.4, standAlone = TRUE)
}

g1 <- ggplot(data = power_endp, aes(x = endpoint, y = power, col = data)) + 
  # geom_vline(xintercept =max(lifetimes$death), alpha = 0.5) + 
  # geom_point(data = power_df, aes(x = endpoint, y = power, col = data)) + 
  geom_rug(data=lifetimes, mapping = aes(x = death, y = NULL, col = data)) + 
  geom_line(data = power_endp, 
            aes(x = endpoint, 
                y = power, col = data),
            size = 1) + 
  geom_line(data = power_endp_combo,
            aes(x = endpoint, y = power), 
            color = "black", lty = 2) + 
  scale_color_viridis_d() + 
  theme_classic() + 
  theme(legend.position = "bottom", panel.grid.major = element_line())  + #seq(0,1, by = 0.1)) +
  ...
  scale_x_continuous(breaks = seq(120, 160, by = 20),
                     limits = c(min(endpoints),160),
                     expand = c(0,0),
                     labels = paste0("$",seq(120, 160, by = 20),"$")) + 
  xlab("upper limit to human lifespan") 
g2 <- ggplot(data = power_gamma_wald, aes(x = shape, y = power, col = data)) + 
  geom_hline(yintercept = 0.05, alpha = 0.5) + 
  geom_smooth(method = "gam", se = FALSE,
              formula = y ~ s(x, bs = "tp", fx = TRUE, k=15)) +
  geom_line(data = power_gamma_wald_combo,
            aes(x = gamma, y = power), 
            color = "black", lty = 2) + 
  scale_color_viridis_d() + 
  theme_classic() + 
  theme(legend.position = "none", panel.grid.major = element_line())  + #seq(0,1, by = 0.1)) +
  scale_y_continuous(expand = c(0,0), 
                     limits = c(0,1.01),
                     breaks = seq(0,1, by= 0.25),
                     labels = c("$0$","$0.25$","$0.5$", "$0.75$", "$1$")) +
...
# Compute the power and combined power (see main text)
round(rbind(
